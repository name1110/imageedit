...
import com.name1110.photoeditor.databinding.ActivityAddImageBinding
import com.name1110.photoeditor.model.MyImages
import com.name1110.photoeditor.util.ControlPermission
...
class AddImageActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        addImageBinding.btnedit.setOnClickListener {
            ...
            if (::selectedImage.isInitialized) {
                // 如果已经选择了图片，则启动编辑界面
                val compressedimage = compressImage(selectedImage,400,350,100)
    ...
    fun compressImage(bitmap: Bitmap, maxWidth: Int, maxHeight: Int, quality: Int): Bitmap {
        ...
        if (width > maxWidth || height > maxHeight) {
            ...
            scale = if (widthRatio > heightRatio) {
        ...
        // Create matrix for the scaling and rotate the bitmap if needed
        val matrix = Matrix()
        matrix.postScale(scale, scale)

        // Recreate the new Bitmap and compress it to reduce the size
        ...
        // Return the resized and compressed bitmap
